  ...
  Analytics: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      getUserProperties: function getUserProperties() {
        this.userProperties = {};
        var uuid = cc.Mgr.Config.isTelegram ? window.webapp.initDataUnsafe.user.id : "Local";
  ...
  Payment: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      purchase: function purchase(_productId) {
        ...
        cc.Mgr.game.pauseGame();
        cc.Mgr.http.httpPost("https://tg-api-service.lunamou.com/orders/create", requestBody, function(error, response) {
          if (true == error) {
          ...
          if (data && data.invoice_url) window.webapp.openInvoice(data.invoice_url, function(status) {
            "paid" === status ? _this.checkOrderStatus(data.id) : "failed" === status ? window.webapp.showAlert("Payment failed. Please try again.") : "cancelled" === status ? window.webapp.showAlert("Payment was cancelled.") : window.webapp.showAlert("Unexpected payment status: " + status);
          }); else {
            cc.Mgr.UIMgr.showPrompt(cc.Mgr.Utils.getTranslation("payment-failed"), "", _this.tipParent);
            cc.Mgr.UIMgr.hideLoading();
            _this.callback = null;
      ...
      checkOrderStatus: function checkOrderStatus(orderId) {
        ...
        cc.Mgr.http.httpGets("https://tg-api-service.lunamou.com//orders/" + orderId + "/status", function(error, response) {
          ...
          }, 5e3) : window.webapp.showAlert("Order status: " + data.status + ". Please contact support if you have any questions.");
        });
      },
  ...
  shareUI: [ function(require, module, exports) {
    ...
    var shareUI = cc.Class({
      ...
      onClickShare: function onClickShare() {
        ...
        cc.Mgr.UIMgr.showLoading();
        var self = this;
        cc.Mgr.Utils.getBase64Image("resources/tex/shareImage_5.png", function(_data) {
          cc.Mgr.UIMgr.hideLoading();
          cc.Mgr.game.updateMissionProgressById(MissionType.InviteCount);
          cc.Mgr.game.updateAchieveProgressByType(AchieveType.Invite);
          cc.Mgr.UIMgr.InGameUI.checkMissionAchieveTip();
